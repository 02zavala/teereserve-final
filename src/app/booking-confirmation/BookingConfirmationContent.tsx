...
export function BookingConfirmationContent()
 {
  ...
  useEffect(() => {
    ...
    if (bookingId || folio) {
      fetchBookingDetails(bookingId, folio)
    } else {
      ...
      setBooking({
        ...
        courseImage: '/images/courses/cabo-real-1.jpg',
        date: '2025-07-15',
        time: '09:00',
        players: 4,
        totalAmount: 560,
        customerName: 'Oscar Gómez',
        customerEmail: 'oscar@teereserve.golf',
        customerPhone: '+52 555 987 6543',
        comments: 'Reserva de prueba para validar el sistema completo',
        status: 'confirmed',
  ...
  const fetchBookingDetails = async (bookingId: string | null, folio: string | null) => {
    try {
      const response = await fetch(`/api/bookings/${bookingId || folio}`)
      if (response.ok) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-golf-beige-50 to-golf-beige-100 dark:from-gray-900 dark:to-gray-800">
        ...
        <div className="flex items-center justify-center min-h-[60vh]">
          <div className="animate-spin rounded-full h-16 w-16 border-b-2 border-golf-green-600"></div>
        </div>
      </div>
    )
  }
